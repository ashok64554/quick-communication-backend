...
class ChatbotEngine
{
     ...
     */
    protected function goToStep($session, $payload, $stepId)
    {
        $nextStep = collect($payload['steps'])->firstWhere('id', $stepId);

        if ($nextStep) {
            // Detect infinite loop (step pointing to itself)
            if ($session->current_step == $stepId) {
                $session->loop_count = ($session->loop_count ?? 0) + 1;

                if ($session->loop_count >= 3) { // limit set to 3
                    $this->sendReply($session->customer_number, "Session ended due to repeated loop.");
                    $session->delete();
                    return;
                }
            } else {
                // reset counter when moving forward normally
                $session->loop_count = 0;
            }

            $session->current_step = $nextStep['id'];
            $session->save();
            $this->processStep($nextStep, $session, $payload);

        } else {
            $session->delete();
            $this->sendReply($session->customer_number, "Session ended.");
        }
    }
